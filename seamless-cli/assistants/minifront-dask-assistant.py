...
async def run_transformation_dask(
    transformation_checksum, tf_dunder, fingertip, scratch
):
    import os
    import aiohttp
    ...
    for wait_a_second in range(30):
        ...
        raise RuntimeError(
            f"Socket {socket_path} was not created by worker startup script"
        )
            ...
            try:
                while 1:
                    async with session.put(
                        url="http://localhost/", json=data, timeout=timeout
                    ) as response:
...
class SeamlessWorkerPlugin(WorkerPlugin):
    def setup(self, worker):
        ...
        if "SEAMLESS_TRANSFORMATION_SOCKET" not in os.environ:
            raise RuntimeError(
                """The Dask minifront assistant requires SEAMLESS_TRANSFORMATION_SOCKET in the deployment script.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="""Mini-dask assistant.
